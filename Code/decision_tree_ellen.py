...
def get_entropy(y_predict, y_real):
    ...
    s_true, n_true = entropy_of_one_division(
        y_real[y_predict])  # left hand side entropy
    s_false, n_false = entropy_of_one_division(
        y_real[~y_predict])  # right hand side entropy
...
def data_preparation(dataset):
    ...
    dataset.Embarked.fillna(
        value=(dataset.Embarked.value_counts().idxmax()), inplace=True)
    enc = OneHotEncoder()
    enc.fit(dataset)
    ...
    def fit(self, x, y, feature_names, par_node={}, depth=0):
        if par_node is None:
            ...
            par_node['left'] = self.fit(
                x[x[:, col] < cutoff], y_left, feature_names, {}, depth+1)
            par_node['right'] = self.fit(
                x[x[:, col] >= cutoff], y_right, feature_names, {}, depth+1)
            self.depth += 1
            if self.tree == None:
                self.tree = par_node
            return par_node
    ...
    def _get_prediction(self, row):
        ...
        while cur_layer.get('cutoff'):
            if row[cur_layer['index_col']] < cur_layer['cutoff']:
                #catching nodes past the leaf node
...
X_train, X_test, y_train, y_test = train_test(
    dataset, target_name, train_size=0.8, shuffle=True)
...
print('Ellens Decision Tree has a prediction accuracy of ', accuracy, '%')
...
class RandomForestClassifier(object):
    ...
    def fit(self, dataset, target_name, shuffle):
        ...
        for i in range(self.n_trees):
            X_train, X_test, y_train, y_test = train_test(
                dataset, target_name, train_size=self.sample_size, shuffle=shuffle)
            clf = DecisionTreeClassifier(max_depth=5, min_samples_split=10)
...
class RandomForest(object):
    def __init__(self, sample_size, n_trees, min_samples_split, max_depth, max_features=None):
        self.depth = 0
        self.max_depth = max_depth
        self.min_samples_split = min_samples_split
        self.n_trees = n_trees
    ...
    def create_tree(self):
        return DecisionTreeClassifier(min_samples_split = self.min_samples_split, max_depth = self.max_depth)
    
    def fit(self, X_train, y_train, feature_names):   
        trees = list()
        for tree in self.trees:
            tr = tree.fit(X_train, y_train, feature_names)
            trees.append(tr)
        return trees
    
    def predict(self, x):
        #Average of the predictions from each tree
        prediction_set = np.array([0]*self.n_trees)
        for i, tree in enumerate(self.trees):
            prediction_set[i] = tree.predict(x)
        results = max(set(prediction_set), key = prediction_set.count)
        return results
...
rf = RandomForest(max_depth = 3, min_samples_split = 10, n_trees = 3, sample_size = 0.8)
print(rf.trees)
