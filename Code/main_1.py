...
def main():
	...
	np.save('/home/z5209394/Data/train_all.npy', dataset)
	np.save('/home/z5209394/Data/test_all.npy', dataset_test)
	np.save('/home/z5209394/Data/y_train_all.npy', Y_train)
	np.save('/home/z5209394/Data/y_test_all.npy', Y_test)
	...
	for iter in range(len(dataset_test)):
		input_image = dataset_test[iter][7][:, 15, :]*255
		filterSize =(16,16)
		kernel = cv2.getStructuringElement(cv2.MORPH_RECT,filterSize)
		tophat_img = cv2.morphologyEx(input_image, cv2.MORPH_BLACKHAT,kernel)
		arr = tophat_img[(tophat_img > 105) & (tophat_img < 170)]
		if len(arr) > 0:
			X_test_filtered.append(dataset_test[iter])
	...
	# Classifier predict
	predictions = (classifier.predict_proba(X_test_filtered)[:,1] >= 0.15).astype(bool)
	Y_test_filtered=Y_test_filtered.reshape((len(Y_test_filtered),1))
	print(confusion_matrix(Y_test_filtered, predictions))

	predictions = (classifier.predict_proba(X_test_filtered)[:,1] >= mean_thresh).astype(bool)
	Y_test_filtered=Y_test_filtered.reshape((len(Y_test_filtered),1))
	print(confusion_matrix(Y_test_filtered, predictions))

	predictions = (classifier.predict_proba(X_test_filtered)[:,1] >= 0.5).astype(bool)
	Y_test_filtered=Y_test_filtered.reshape((len(Y_test_filtered),1))
	print(confusion_matrix(Y_test_filtered, predictions))
